...
export async function cacheReview(
  ...
  const hash = crypto
    .createHash("md5")
    .update(`${originalReview}\x00${targetLanguage}`)
    .digest("hex");
  const sourceLanguage = franc(originalReview).slice(0, 2);
  await client.set<string>(`review:${hash}`, translatedReview, {
    ex: CACHED_REVIEW_EXPIRY,
  });
  await client.set<string>(`review:${hash}:source_lang`, sourceLanguage, {
    ex: CACHED_REVIEW_EXPIRY,
  });
